...
rav::sdp::SourceFilter::parse_new(const std::string_view line) {
    ...
    // Skip leading space
    if (!parser.skip(' ')) {
        return ParseResult<SourceFilter>::err("source_filter: leading space not found");
    }

    // Filter mode
    const auto filter_mode = parser.split(' ');
    if (!filter_mode) {
        return ParseResult<SourceFilter>::err("source_filter: filter mode not found");
    }
    if (filter_mode == "incl") {
        ...
        return ParseResult<SourceFilter>::err("source_filter: invalid filter mode");
    }

    // Network type
    const auto netw_type = parser.split(' ');
    if (!netw_type) {
        return ParseResult<SourceFilter>::err("source_filter: network type not found");
    }

    if (netw_type == k_sdp_inet) {
        filter.net_type_ = NetwType::internet;
    } else {
        return ParseResult<SourceFilter>::err("source_filter: invalid network type");
    }

    // Address type
    const auto addr_type = parser.split(' ');
    if (!addr_type) {
        return ParseResult<SourceFilter>::err("source_filter: address type not found");
    }
    ...
    if (addr_type == k_sdp_ipv4) {
        ...
        return ParseResult<SourceFilter>::err("source_filter: invalid address type");
    }

    // Destination address
    const auto dest_address = parser.split(' ');
    if (!dest_address) {
        return ParseResult<SourceFilter>::err("source_filter: destination address not found");
    }

    if (dest_address->empty()) {
        return ParseResult<SourceFilter>::err("source_filter: destination address is empty");
    }
    ...
    for (auto i = 0; i < RAV_LOOP_UPPER_BOUND; ++i) {
        ...
        if (src_address->empty()) {
            return ParseResult<SourceFilter>::err("source_filter: source address is empty");
        }
        filter.src_list_.emplace_back(*src_address);
    }

    return ParseResult<SourceFilter>::ok(filter);
}
