...
TEST_CASE("rtp_packet_stats") {
    ...
    SECTION("Drop one packet") {
        ...
        // Move the window so that 11 (which didn't arrive) moves out of the window
        stats.update(13);
        stats.update(14);
    ...
    SECTION("Handling maximum window size") {
        {
            rav::rtp_packet_stats stats(0x7fff);
            stats.update(0);
            stats.update(32767);
            auto totals = stats.get_total_counts();
            REQUIRE(totals.dropped == 0);
