...
TEST_CASE("sliding_stats") {
    ...
    SECTION("median") {
        ...
        REQUIRE(rav::util::is_within(stats.median(), 0.0, 0.0));
        stats.add(1);
        REQUIRE(stats.count() == 1);
        REQUIRE(rav::util::is_within(stats.median(), 1.0, 0.0));
        stats.add(500);
        stats.add(4);
        stats.add(3);
        REQUIRE(stats.count() == 4);
        REQUIRE(rav::util::is_within(stats.median(), 3.5, 0.0));
        stats.add(2);
        REQUIRE(stats.count() == 5);
        REQUIRE(rav::util::is_within(stats.median(), 3.0, 0.0));
    }
}
