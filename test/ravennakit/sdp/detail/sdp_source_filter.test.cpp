...
TEST_CASE("source_filter", "[source_filter]") {
    SECTION("Example 1") {
        auto result = rav::sdp::SourceFilter::parse_new(" incl IN IP4 232.3.4.5 192.0.2.10");
    ...
    SECTION("Example 2") {
        auto result = rav::sdp::SourceFilter::parse_new(" excl IN IP4 192.0.2.11 192.0.2.10");
        REQUIRE(result.is_ok());
        auto filter = result.move_ok();
    ...
    SECTION("Example 3") {
        auto result = rav::sdp::SourceFilter::parse_new(" incl IN IP4 * 192.0.2.10");
        REQUIRE(result.is_ok());
        auto filter = result.move_ok();
        REQUIRE(filter.mode() == rav::sdp::FilterMode::include);
        REQUIRE(filter.network_type() == rav::sdp::NetwType::internet);
    ...
    SECTION("Example 4") {
        auto result = rav::sdp::SourceFilter::parse_new(" incl IN IP6 FF0E::11A 2001:DB8:1:2:240:96FF:FE25:8EC9");
        REQUIRE(result.is_ok());
        auto filter = result.move_ok();
        REQUIRE(filter.mode() == rav::sdp::FilterMode::include);
        REQUIRE(filter.network_type() == rav::sdp::NetwType::internet);
    ...
    SECTION("Example 5") {
        auto result = rav::sdp::SourceFilter::parse_new(" incl IN * dst-1.example.com src-1.example.com src-2.example.com");
        REQUIRE(result.is_ok());
        auto filter = result.move_ok();
        REQUIRE(filter.mode() == rav::sdp::FilterMode::include);
        REQUIRE(filter.network_type() == rav::sdp::NetwType::internet);
